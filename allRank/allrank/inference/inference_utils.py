...
def __rank_slates(dataloader: DataLoader, model: LTRModel) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    reranked_X = []
    reranked_y = []
    scores = []
    model.eval()
    ...
    with torch.no_grad():
        for xb, yb, _ in dataloader:
            ...
            scores_batch = model.score(X, mask, input_indices)
            
            scores_batch[mask] = float('-inf')

            _, indices = scores_batch.sort(descending=True, dim=-1)
            ...
            # Append scores_batch to the scores list after moving it to CPU
    ...
    return combined_X, combined_y, scores
