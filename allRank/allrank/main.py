...
def run():
    ...
    '''
    slates_X, slates_y, scores = __rank_slates(val_dl, model)
    np.savetxt("predictions_c2te29.txt", slates_y, delimiter='\t')
    # Assuming scores is your list of tensors containing scores
    scores_concatenated = torch.cat(scores, dim=0)  # Concatenate tensors along the batch dimension

# Convert the concatenated tensor to a numpy array and then save it
    scores_np = scores_concatenated.cpu().numpy()
    np.savetxt("scores_c2te29.txt", scores_np, delimiter='\t')
    '''
